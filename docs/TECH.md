...
ASR 选**本地 faster-whisper**（small/int8/CPU，实测单条 ~3s）而非云端：讲述内容高度敏感，音频不出机器（与日志脱敏同理）；且用户的中转站不提供转写接口（2026-07-27 实测 404）。

#### 3.5.9 低门槛表达（换掉"必须打字"的前提）

把"选择题比填空题容易答"贯穿输入侧，三层递进：

1. **话题卡牌开局**（StoryCorps Great Questions 方法）：8 张卡（一件旧物/一通电话/一句没说出口的话…），每张自带定制开场白，选卡即入题；也可不选。卡牌与开场白定义在 `agent/cards.py`。
2. **快捷回应 chips**：每轮回复后由 LLM 生成 2~3 个第一人称候选回应（顺答/带情绪/引向深处）+固定的"换个话题"，点选即发送（`prompts/suggest_replies.md`；SSE 在 done 后追加 suggestions 事件，不拖慢正文）。
3. **按住说话**：见 §3.5.8。

#### 3.5.10 读者评审团（多 agent 共鸣反馈）

三个读者 persona（打工人/母亲/学生）**不共享上下文**并行读故事（对等评审，避免互相污染），各自输出"共鸣与否 + 一句话反应"，结果并列呈现不合并投票——把 PRD 的"共鸣"从愿景变成生成故事稿后 5 秒内可见的反馈（`agent/reactions.py` + `prompts/reader_panel.md`）。

#### 3.5.11 persona 自动评估环

`backend/eval/`：3 个模拟受访者（寡言/跑题/防御心重，各藏一个真实故事）通过真实 HTTP 接口与访谈 agent 对话 N 轮 → 生成故事稿 → LLM 裁判按 6 维打分（一次一问/先接住/不说教/细节挖掘/可溯源/打动力）。提示词调优前后各跑一次对比——没有评估，就没有进步。用法：`conda run -n ai_agent python -m eval.run_eval --turns 5`（需后端已启动）。
...
| 2026-07-27 | v0.3 新增 §3.5.8~11 | 低门槛表达三件套（卡牌/chips/语音）、本地 faster-whisper ASR、读者评审团（多 agent）、跨会话长期记忆（可删除）、persona 评估环落地 |
